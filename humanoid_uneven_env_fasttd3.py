...
def make_env(
    ...
    if env_name == "HumanoidWalk":
        train_env_cfg = default_config()
        train_env_raw = Humanoid(
            move_speed=1.0,
            config=train_env_cfg
        )

    train_env_cfg_default = registry.get_default_config(env_name)
    ...
    train_env = wrapper_torch.RSLRLBraxWrapper(
        ...
        num_envs,
        seed,
        train_env_cfg_default.episode_length,
        train_env_cfg_default.action_repeat,
    ...
    if env_name == "HumanoidWalk":
        eval_env_cfg = default_config()
        eval_env_raw = Humanoid(
            move_speed=1.0,
            config=train_env_cfg
        )

    eval_env_cfg_default = registry.get_default_config(env_name)

    eval_env = PlaygroundEvalEnvWrapper(
        eval_env_raw,
        eval_env_cfg_default.episode_length,
        env_name,
        num_eval_envs,
        seed,
        device_rank=device_rank,
    ...
    if env_name == "HumanoidWalk":
        render_env_cfg = default_config()
        render_env_raw = Humanoid(
            move_speed=1.0,
            config=train_env_cfg
        )
    render_env_cfg_default = registry.get_default_config(env_name)

    render_env = PlaygroundEvalEnvWrapper(
        render_env_raw,
        render_env_cfg_default.episode_length,
        env_name,
        1,
        seed,
        device_rank=device_rank,
